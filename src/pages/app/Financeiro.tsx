  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: s }, { data: ps }, { data: o }, { data: e }, { data: sp }, { data: ti }] = await Promise.all([
        supabase.from("sales").select("id,sale_number,total,net_value,created_at,payment_status,due_date,customer_name,customer_whatsapp,payment_method").eq("store_id", store.id).neq("status", "estornada").gte("created_at", fromIso).lte("created_at", toIso).order("created_at", { ascending: false }),
