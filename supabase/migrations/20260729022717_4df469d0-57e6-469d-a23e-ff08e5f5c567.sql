-- 1) Marcação de categorias que representam compra de estoque (não despesa operacional)
ALTER TABLE public.expense_categories
  ADD COLUMN IF NOT EXISTS is_stock_purchase boolean NOT NULL DEFAULT false;

UPDATE public.expense_categories
   SET is_stock_purchase = true
 WHERE lower(name) IN ('compra de mercadorias', 'compras / mercadorias', 'compra de mercadoria', 'compras/mercadorias');

-- 2) Dashboard: despesas operacionais excluem compras de estoque
CREATE OR REPLACE FUNCTION public.get_dashboard_metrics(_store_id uuid, _from timestamp with time zone, _to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  -- Despesas do período separadas entre operacionais e compra de estoque.
  -- Compra de mercadoria NÃO é despesa: vira custo (CMV) quando o item é vendido.
  WITH exp AS (
