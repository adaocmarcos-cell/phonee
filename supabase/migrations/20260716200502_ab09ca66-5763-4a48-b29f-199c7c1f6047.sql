...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- Allow admin master or trusted callers (pg_cron runs as table owner, auth.uid() is null)
  ...
  -- 1) Create missing stores for trials whose user has no owned store
    ...
    SELECT ms.*,
      COALESCE(
        NULLIF(regexp_replace(
          lower(unaccent(coalesce(ms.store_name, ms.full_name, split_part(ms.email,'@',1), 'loja'))),
          '[^a-z0-9]+', '-', 'g'
        ), ''),
        'loja'
      ) AS base_slug
  ...
  inserted AS (
    ...
    SELECT
      ...
      -- if slug clashes, append short suffix
      CASE
  ...
  -- Only log when something was actually corrected
...
REVOKE ALL ON FUNCTION public.backfill_trial_orphans() FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.backfill_trial_orphans() TO authenticated, service_role;
