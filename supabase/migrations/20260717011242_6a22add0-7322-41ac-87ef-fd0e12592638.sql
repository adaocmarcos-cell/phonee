...
-- Fila "Aguardando preparo" + consumo automático de peças + custo manual de reparo
CREATE OR REPLACE FUNCTION public.finish_trade_in_repair(
  _trade_in_id uuid,
  _parts jsonb DEFAULT '[]'::jsonb,      -- [{part_id, qty, unit_cost, name}]
  _manual_cost numeric DEFAULT 0,
  _manual_notes text DEFAULT NULL
) RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Valida disponibilidade e baixa estoque
...
GRANT EXECUTE ON FUNCTION public.finish_trade_in_repair(uuid, jsonb, numeric, text) TO authenticated;
