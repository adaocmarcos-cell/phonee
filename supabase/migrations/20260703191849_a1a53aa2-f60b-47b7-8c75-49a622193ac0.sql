...
CREATE OR REPLACE FUNCTION public.create_sale(
  ...
  _discount numeric,
  _notes text,
  _items jsonb,     -- [{product_id, quantity, unit_price, is_service?, description?, name?, sku?, category?, brand?, model?, unit?, discount_amount?, warranty_days?, imei_serial?}]
  _payments jsonb   -- [{method, amount, installments?, notes?}]
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Autorização: dono da loja OU membro vinculado
  ...
  -- Cabeçalho da venda (subtotal e total serão recalculados no banco)
  ...
  -- Itens: recalcula preços no banco e baixa estoque com trava (produtos físicos)
  FOR v_item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    IF NOT v_is_service THEN
      ...
      -- Baixa de estoque atômica: só passa se houver saldo
  ...
  -- Total recalculado no banco (frontend não é confiável)
  ...
  -- Pagamentos: soma tem que fechar com o total
...
REVOKE ALL ON FUNCTION public.create_sale(uuid,uuid,text,text,text,public.payment_method,int,numeric,text,jsonb,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_sale(uuid,uuid,text,text,text,public.payment_method,int,numeric,text,jsonb,jsonb) TO authenticated;
