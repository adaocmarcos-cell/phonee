...
CREATE OR REPLACE FUNCTION public.stock_products_page(
  ...
  _filter text DEFAULT 'all'::text,
  _brand text DEFAULT 'all'::text,
  _category text DEFAULT 'all'::text,
  _page integer DEFAULT 1,
  _page_size integer DEFAULT 20
)
RETURNS TABLE(
  id uuid, name text, sku text, brand text, category text, condition text, status text,
  cost_price numeric, sale_price numeric, stock_current integer, stock_min integer,
  last_sold_at timestamptz, supplier text, item_kind text, total_count bigint
)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    p.category::text, p.condition::text, p.status::text,
    p.cost_price, p.sale_price, p.stock_current, p.stock_min,
    p.last_sold_at, p.supplier, p.item_kind::text,
...
REVOKE ALL ON FUNCTION public.stock_products_page(uuid, text, text, text, text, integer, integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.stock_products_page(uuid, text, text, text, text, integer, integer) TO authenticated, service_role;